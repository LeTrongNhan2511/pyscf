...
def kernel(mp, mo_energy=None, mo_coeff=None, eris=None, with_t2=WITH_T2,
   ...
   # mp.second_order = False
    ...
    #initial dft:
    #ks= dft.UKS(mol,f"HF,{(1-alpha)}*LYP").density_fit()
    #print('DFT part') 
    ks= dft.UKS(mol,f"0.53*HF+ 0.47*B88,{(1-alpha)}*LYP").density_fit()
    ...
    for it in range(niter):
        ...
        # Total energy
        ene_tot =  (ene_dft) + (ene_uobmp2 - ene_hfpyscf) * alpha
        
        # Fock
        fock_udftobmp2_a = (fock_dft_a) + (fock_uobmp2_a - fock_hf_pyscf_a) *alpha
        fock_udftobmp2_b = (fock_dft_b) + (fock_uobmp2_b - fock_hf_pyscf_b) *alpha
        ...
        # mo_energy[0], U = scipy.linalg.eigh(fock_udftobmp2_a)
        # mo_coeff[0] = numpy.matmul(mp.mo_coeff[0], U)
        # mo_energy[1], U = scipy.linalg.eigh(fock_udftobmp2_b)
        # mo_coeff[1] = numpy.matmul(mp.mo_coeff[1], U)
        ##
...
class UB2PLYPDFUOBMP2(dfobmp2_faster_ram.DFOBMP2):
    ...
    def kernel(self, mo_energy=None, mo_coeff=None, eris=None, with_t2=WITH_T2, _kernel=kernel):
        ...
        # self.ene_tot =_kernel(self, mo_energy, mo_coeff, eris, with_t2, alpha)
        
        if hasattr(self, 'alpha'):
            self.ene_tot, self.dm, self.ss, self.tmp1_bar =_kernel(self, mo_energy, mo_coeff, eris, with_t2, self.alpha)
        else:
            self.ene_tot, self.dm, self.ss, self.tmp1_bar =_kernel(self, mo_energy, mo_coeff, eris, with_t2,  verbose=logger.NOTE)

        return self.ene_tot, self.dm, self.ss, self.tmp1_bar
